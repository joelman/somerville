...
addresses = []
sql = 'select sum([sale price]) price, sum([parcel val]) value, [PARCEL ID], [HOUSE NO], [STREET], [ZONE DESP], [WARD], lat, lon from property p where [fiscal_year] = (select max([fiscal_year]) from property where [parcel id] = p.[parcel id]) and street <> 0 and lat is not null group by [PARCEL ID], [house no], [street], [zone desp], lat, lon order by [street], [HOUSE NO]'

c.execute(sql)
for row in c.fetchall():
    line = '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' % (row['price'], row['value'], row['PARCEL ID'], row['HOUSE NO'], row['STREET'], row['ZONE DESP'], row['WARD'], row['lat'], row['lon'])
